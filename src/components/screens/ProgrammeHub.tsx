...
export function ProgrammeHub({ userProfile, generatedProgrammes, activeProgrammeId, onNavigate, onViewProgramme, onDeleteProgramme }: Props) {
  ...
  const activeProg = generatedProgrammes.find(p => p.id === activeProgrammeId) ?? null;
  const previousProgs = generatedProgrammes.filter(p => p.id !== activeProgrammeId);

  const renderCard = (prog: GeneratedProgramme, showActiveTag: boolean) => {
